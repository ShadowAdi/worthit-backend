...
class BrandControllerClass {
    ...
    async getBrandBySlug(req, res, next) {
        try {
            ...
            const { brand, reviews } = await BrandService.getBrandBySlug(slug, userId);
            if (!brand) {
    ...
    async getBrandByNameOrSlug(req, res, next) {
        try {
            ...
            const brands = await BrandService.getBrandByNameOrSlug(identifier);
    ...
    async deleteBrand(req, res, next) {
        try {
            ...
            const message = await BrandService.deleteBrand(brandId, userId);
    ...
    async updateBrand(req, res, next) {
        try {
            ...
            const payload = req.body;
            const updatedBrand = await BrandService.updateBrand(brandId, userId, payload);
            res.status(200).json({
                success: true,
    ...
    async increaseBrandView(req, res, next) {
        try {
            const { brandId } = req.params;
            if (!brandId) {
                logger.error(`Brand ID not provided`);
            ...
            const updatedBrand = await BrandService.increaseBrandView(brandId);
    ...
    async updateBrandTeam(req, res, next) {
        try {
            const userId = req.user?.id;
            const { brandId } = req.params;
            const { team } = req.body;
            ...
            const updatedBrand = await BrandService.updateBrandTeam(brandId, userId, team);
