...
class BrandClassService {
    async createBrand(payload, userId) {
        try {
    ...
    async getBrand(brandId) {
        try {
            ...
            const errorMessage = error instanceof Error ? error.message : 'Unknown error';
            logger.error(`Failed to get brand by id:${brandId} error: ${errorMessage}`);
            throw new AppError(errorMessage, 500);
        }
    }
    async getBrandBySlug(slug, userId) {
        try {
            ...
            const isCreator = userId && brand.founderId && (userId === brand.founderId._id?.toString() ||
            ...
            const errorMessage = error instanceof Error ? error.message : 'Unknown error';
            logger.error(`Failed to get brand by slug:${slug} error: ${errorMessage}`);
            throw new AppError(errorMessage, 500);
        }
    }
    async getBrandByNameOrSlug(identifier) {
        try {
    ...
    async updateBrand(brandId, userId, updateBrandPayload) {
        try {
            ...
            if (!brandExists) {
                throw new AppError("Brand not found or you are not authorized", 403);
            }
            if (updateBrandPayload.launchAt &&
                new Date(updateBrandPayload.launchAt) < new Date()) {
                throw new AppError("Launch date cannot be in the past", 400);
            }
            const updateData = {};
            if (updateBrandPayload.name !== undefined)
                updateData.name = updateBrandPayload.name;
            if (updateBrandPayload.description !== undefined)
                updateData.description = updateBrandPayload.description;
            if (updateBrandPayload.slug !== undefined)
                updateData.slug = updateBrandPayload.slug;
            if (updateBrandPayload.one_liner !== undefined)
                updateData.one_liner = updateBrandPayload.one_liner;
            if (updateBrandPayload.whyItExists !== undefined)
                updateData.whyItExists = updateBrandPayload.whyItExists;
            if (updateBrandPayload.demoVideo !== undefined)
                updateData.demoVideo = updateBrandPayload.demoVideo;
            if (updateBrandPayload.category !== undefined)
                updateData.category = updateBrandPayload.category;
            if (updateBrandPayload.howToUse !== undefined)
                updateData.howToUse = updateBrandPayload.howToUse;
            if (updateBrandPayload.websiteUrl !== undefined)
                updateData.websiteUrl = updateBrandPayload.websiteUrl;
            if (updateBrandPayload.launchAt !== undefined)
                updateData.launchAt = updateBrandPayload.launchAt;
            ...
            const updatedBrand = await Brand.findByIdAndUpdate(brandId, { $set: updateData }, {
                new: true,
                runValidators: true
            })
                .populate("founderId", "username email profile_url")
    ...
    async increaseBrandView(brandId) {
        try {
    ...
    async updateBrandTeam(brandId, userId, teamMembers) {
        try {
            const brandExists = await Brand.exists({
                _id: brandId,
                founderId: userId
            ...
            const updatedBrand = await Brand.findByIdAndUpdate(brandId, { $set: { team: formattedTeam } }, {
                new: true,
                runValidators: true
            })
                .populate("founderId", "username email profile_url")
